...
int remuneracaoSemanal = Pagamento.ValorPagamento(retorno.valorHora, retorno.horasPagar);
Console.WriteLine(remuneracaoSemanal);

Console.WriteLine($"O horista {horista} trabalhou {retorno.horasTotais}h, sendo {retorno.horas}h normais, {retorno.horasSabado}h a 50% e {retorno.horasDomingo} a 100% totaliando {retorno.horasEfetivas}h efetivas sendo consideradas {retorno.horasPagar}h a pagar e {retorno.horasBanco}h como banco de horas. Com o valor hora de R$ {retorno.valorHora} a remuneração final é de {remuneracaoSemanal}");
